...
Anthropic reports 5-hour and 7-day shared windows, plus Sonnet, Opus and OAuth-app windows when returned. Newer responses may also include a model-scoped `limits` entry (for example a Fable weekly limit); active model-scoped entries are treated as that model family's window, shown as `7d <Model>` (or `5h` for session-scoped entries) and included in scoring only when the active model belongs to that family. The details view shows each supported window's remaining percentage and reset time. Scoring uses the lowest remaining percentage across shared windows (including OAuth apps, when present) and the requested model family's window. `/subs limits` uses the active Anthropic model's family; with another provider active it scores shared windows only. Other families' windows are shown but do not make every model look exhausted. `quota-first` uses the model being routed. Missing or malformed core windows, or malformed applicable optional windows, make overall quota unknown rather than healthy. Missing reset timestamps display as unknown, not an invented reset. Extra-usage billing and unrecognized windows are not included in subscription headroom; no monetary balance or paid-overage availability is inferred.
